(function () {
	 ...
	 * @constructor
	 */
	function EventListener(owner, eventPrefix, isGlobalListener) {
		...
		isGlobalListener = isGlobalListener || false;
		var self = this;
	...
	function Channel(id) {
		...
		this.join = function (fn) {
			...
			socket.emit('channel_join', this.getId(), function (data, err) {
				isJoined = !err;
				if (err) {
	...
	function YiiNodeSocket() {
		...
		var globalEventListener = new EventListener(this, '', true);
