		...
		join : function (socket, fn, isClientSide) {
			...
			if (socket.handshake['uid']) {
				if (this.subscribers[socket.handshake['uid']]) {
					...
					var subscriber = subscribers[socket.handshake['uid']];
					if (subscriber && isClientSide && this.attributes.subscriber_source > 1) {
